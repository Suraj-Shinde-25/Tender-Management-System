        ...
        <div class="marquee-content" style="align:center; padding-top:200px;min-height:750px;background-color:cyan">
			...
			<table style="background-color: white">
				<tr
					...
					<td style="padding: 15px; border: 2px solid black">Bidder Id</td>
					<td style="padding: 15px; border: 2px solid black">Vendor Id</td>
					<td style="padding: 10px; border: 2px solid black">Bid Amount</td>
					<td style="padding: 15px; border: 2px solid black">Deadline</td>
					<td style="padding: 15px; border: 2px solid black">Status</td>
					<td style="padding: 15px; border: 2px solid black">Accept</td>
					<td style="padding: 15px; border: 2px solid black">Reject</td>
				</tr>
				<%
				...
				<tr>
					<td style="padding: 5px; border: 2px solid black;"><%=bidder.getBidId()%></td>
					<td style="padding: 5px; border: 2px solid black;"><a
						href="adminViewVendorDetail.jsp?vid=<%=bidder.getVendorId()%>"><%=bidder.getVendorId()%></a></td>
					<td style="padding: 5px; border: 2px solid black;"><%=bidder.getBidAmount()%></td>
					<td style="padding: 5px; border: 2px solid black;"><%=bidder.getBidDeadline()%></td>
					<td style="padding: 5px; border: 2px solid black;"><%=bidder.getBidStatus()%></td>
					...
					<td style="padding: 5px; border: 2px solid black;"><a
						href="AcceptBidSrv?bid=<%=bidder.getBidId()%>&tid=<%=bidder.getTenderId()%>&vid=<%=bidder.getVendorId()%>"><button
								class="btn btn-success" style="padding: 5px 15px;">Accept</button></a></td>
					<td style="padding: 5px; border: 2px solid black;"><a
						href="RejectBidSrv?bid=<%=bidder.getBidId()%>&tid=<%=bidder.getTenderId()%>&vid=<%=bidder.getVendorId()%>">
						<button class="btn btn-danger" style="padding: 5px 15px;">Reject</button></a></td>

					<%
					} else {
					%>

					<td style="padding: 5px; border: 2px solid black;"><button class="btn btn-success" disabled><%=status%></button></td>
					<td style="padding: 5px; border: 2px solid black;"><button class="btn btn-danger" disabled><%=status%></button></td>
					<%
					}
					%>
				</tr>
